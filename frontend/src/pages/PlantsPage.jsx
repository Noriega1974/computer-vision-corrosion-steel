...
function PuntoForm({ initial = {}, onSubmit, saving, error }) {
  ...
  useEffect(() => {
    const yaHayCoordenadas =
      initial.coordenadas?.lat ?? initial.latitud;
  ...
  const handleSubmit = (e) => {
    ...
    if (!municipiosDisponibles.includes(form.ciudad)) {
      setValidationError(
        'Selecciona una ciudad válida del departamento elegido.'
      );
      return;
    }
    ...
    >
      ...
      <div style={{ marginBottom: 14 }}>
        ...
        >
          {showManual
            ? 'Ocultar coordenadas'
            : 'Ser más específico con la ubicación'}
        </button>
...
// ─── Panel de detalle del punto de monitoreo ─────────────────────────────────
function PuntoDetail({ punto, onEdit, isAdmin }) {
  ...
  const {
    mediciones,
    loading
  } = useMedicionesPunto(punto?.id_punto);

  const auditoria =
    punto?.historial_cambios ?? [];
              ...
              mediciones
                ...
                .map((m, i) => (
                  ...
                  >
                    ...
                    >
                      {m.area_corroida_pct != null
                        ? `${m.area_corroida_pct.toFixed(1)}%`
                        : '—'}
                    </div>
                  </div>
                ))
...
export default function PlantsPage() {
  ...
  const [selectedPunto, setSelectedPunto] =
    useState(null);
  const [editPunto, setEditPunto] =
    useState(null);
  ...
  return (
    <>
      ...
      {editPunto && (
        <Modal
          title={`Editar: ${editPunto.sede}`}
          onClose={() => setEditPunto(null)}
        >
          <PuntoForm
            initial={editPunto}
            onSubmit={handleEdit}
